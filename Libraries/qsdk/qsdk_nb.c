...
*
*	函数功能：	QSDK 运行环境初始化，初始化at_resp结构体 创建event_mail邮箱，创建hand_thread任务
*
*	入口参数：	无
*
*	返回参数：	0：成功   1：失败
...
int qsdk_init_environment(void)
{
    ...
    //创建事件响应任务 处理NB模组下发的数据 由event_mail邮箱唤醒
...
*	函数名称：	nb_get_time
*
*	函数功能：	获取网络时间，这个时间是模组设置的时间，可更改，要获取ntc时间应该用gps里获取到的时间
*
*	入口参数：	无
*
*	返回参数：	0 成功  1	失败
...
int qsdk_nb_get_net_start(void)
{
    ...
    switch(j)
    {
        case 0:LOG_D("--->没有登录网络,当前没有进行网络搜索"); break;
        case 1:LOG_D("--->已经登录本地网络"); break;
        case 2:LOG_D("--->没有登录网络,当前正在搜索网络"); break;
        case 3:LOG_D("--->注册被拒绝"); break;
        case 4:LOG_D("--->未知状态"); break;
        case 5:LOG_D("--->已经登录网络,处于漫游状态"); break;
        case 6:LOG_D("--->注册到“仅限短信” ,仅当<Act>为 NB-IOT 是才适用"); break;
        case 7:LOG_D("--->注册到“仅限短信”，处于漫游状态。仅当<Act>为 NB-IOT 时才适用"); break;
        default:LOG_D("--->未知错误"); break;
    }
#endif //QSDK_USING_ME3616 end
#endif //QSDK_USING_DBUG end
...
#endif
    rt_thread_mdelay(100);
    if(at_exec_cmd(at_resp,"AT+ZGNMEA=2")!=RT_EOK)	return RT_ERROR;  //2 - RMC
    rt_thread_mdelay(100);
    if(at_exec_cmd(at_resp,"AT+ZGRUN=1")!=RT_EOK)	return RT_ERROR;  //启动单次定位模式
